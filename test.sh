...
#nohup 
# python3 src/main_dyRBT.py train --dynet-gpu 1 --parser-type $model --model-path-base models/$model-model --lstm-dim $lstmdim --label-hidden-dim $lstmdim --split-hidden-dim $lstmdim --pretrainemb $pretrain --batch-size $batchsize --epochs 30 --treetype $treetype --expname $log --normal $normal --checks-per-epoch 4 --RBTlabel $RBTlabel --nontlabelstyle $nontlabelstyle --dropout $dropout --zerocostchunk $zerocostchunk --loadmodel none  >> $log".log" 2>&1 &

python3 src/main_dyRBT.py test --model-path-base models/$model-model --treetype $treetype --expname $log --normal $normal --RBTlabel $RBTlabel 
